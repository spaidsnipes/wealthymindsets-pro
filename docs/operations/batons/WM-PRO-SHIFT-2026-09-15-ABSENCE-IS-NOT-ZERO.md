...
| 14 | `748e661` | `/command-deck` | **The Orkin NEST of #13 — and the first one with NO PIXEL, filed LATENT.** Found by turning #13's law on the *same function* rather than a new screen: two branches down, `if (engaged.length === 0 && permission) clearances.push("No trader rules engaged.")`. With **zero rules configured** that condition is **vacuously true** — nothing could have engaged, so nothing engaging is not a finding. `PermissionVM.ruleCount` carried the denominator the whole time; only this consumer was guessing. Cured **both halves**: `ruleCount === 0` → silence; `ruleCount > 0` → the count **with its denominator**, `0/8 trader rules engaged.` Same gate applied to the `"A HARD trader rule engages."` invalidator — you cannot promise a tripwire that does not exist. **A pre-fix live read shows `2/8 engaged` on the Founder's own account, so the branch never fired on his screen and NO before/after pixel exists.** Filed LATENT, not PROVEN |
