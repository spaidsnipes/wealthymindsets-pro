...
| 5 · Liquidity Weather / Risk Temperature | APPROVED DIRECTION → IN TRANSLATION | `b78f227` | Honest "LIQUIDITY WEATHER · NOT WIRED" chip in wordmark row with dashed border (visual grammar for "capability awaiting real wire"). NO fake heatmap gradient per Living-Pixel Law. Will graduate to RUNTIME MATCH when a licensed L2 depth provider lands and `depthSubscribed` flips true in selectPerCapabilityFidelity. |
| 4 · Evidence Debt / Question Mode | PARTIAL RUNTIME MATCH → **RUNTIME MATCH** | `9377467` | Dedicated "▸ Why" button in wordmark row opens `DecisionWhyPanel` in a `ShellModalDrawer` — SAME panel /command-deck ships. Trader sees the full compiled WHY (not just the count) directly from the primary trading surface. |
| ORDER FLOW capability wire | (cross-primitive) → **RUNTIME MATCH** | `75882a3` | `selectAggressorFlow` output feeds capabilityReport (tapeConnected + orderFlowDerived); when real per-trade ticks arrive the fidelity chip's ORDER FLOW capability lights LIVE. Single derivation, two visible consumers (OrderFlowCockpitStrip + capability report). |
| Wordmark chip overflow | (mobile safety) → **RUNTIME MATCH** | `56cea44` | Row now carries 5+ chips (canvas / liquidity weather / ROW / Why / Passport). Added `overflowX:auto` with hidden scrollbar so chips scroll on narrow devices instead of overlapping the Command Deck link. Spatial memory preserved (no wrap). |
| Sentinel lock — /charts wires | (breadcrumb) → **LOCKED** | `923308a` | 6th consumer breadcrumb in `composeMarketCanvasVM.enforcement.test.ts` verifies ChartsDashboard.tsx keeps: chartCanvasVM.permission → "ROW ·" chip; chartCanvasVM.decisionWhy → DecisionWhyPanel; selectMarketObjectPassport + MarketObjectPassportPanel; LIQUIDITY WEATHER + NOT WIRED honest chip. Future regressions fail loud with file+line. |
