...
**Validated local continuation:** `4099f67` (publication blocked; see Build Status)
...
| Canonical WM Market State | VERIFIED CONTRACT / NOT WIRED | `src/lib/marketData/canonicalMarketState.ts` defines one sealed, outcome-free, availability-aware packet across Direction, Location, Aggression, Regime, Structure, Volatility, Profile and Order Flow (`4099f67`) | Replace hook/component-local ownership with one runtime publisher and selectors; connect chart, heat maps, profiles, Decision Memory, journal and replay without duplicating values |
| Sealed Decision Memory | VERIFIED CONTRACT / NOT WIRED | `src/lib/decisionMemory.ts` seals pre-outcome evidence with time cutoffs and append-only amendments and now requires `wm.market-state.v1` (`08b2b71`, `4099f67`) | Bind to execution/journal flows; persist only after the intended WM database and policies are confirmed |
...
3. Wire the canonical WM Market State contract to one runtime owner referencing Coverage Map facts.
4. Consolidate current Fixed/Session/per-bar controls into one honest WM Profiles surface consuming that same state.
