...
describe("the paper ticket is wired through the guard in the right place", () => {
  const page = readFileSync(path.join(process.cwd(), "src/app/paper/page.tsx"), "utf8");
  const submit = page.slice(page.indexOf("const submit = () => {"));

  it("gates after level validation, before the notice reset and the DECISION_ID mint", () => {
