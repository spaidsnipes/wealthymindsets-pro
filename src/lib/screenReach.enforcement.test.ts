...
const LEDGER: Readonly<Record<string, LedgerEntry>> = {
  ...
  "src/lib/marketData/canonicalBar.ts": {
    reason: "AWAITING_SURFACE",
    note:
      "The one bar contract, from SUPPORT — DECISION_ID Lifecycle + CanonicalBar (2026-09-18) §3: 'no invention ships its own bar builder.' Seventeen files under src/ already declare or destructure an OHLC shape, and three of them — LiveBar in liveBarPolicy.ts, Bar in markov.ts, OHLCVBar in types/index.ts — are the SAME six anonymous fields under three names, which means TypeScript will pass any one where another is expected while nothing in the type says whether that close is a live print, a delayed one, a synthetic aggregate, or the last number a closed session left behind. It has no screen yet because adopting it means migrating those seventeen call sites, and doing that in the same commit that declares the contract would make the contract unreviewable. What a human loses meanwhile is nothing they had. What the house gains now is that asOf and receivedAt exist as two fields instead of one 'time' — the chart cannot render the trader's morning at the end of their afternoon — that a corrected bar gets a NEW id rather than silently overwriting the one the trader already acted on, and that there is deliberately no fromLegacyTuple(): a function that manufactures a symbolId, fidelity, source and provenance out of six loose numbers is the most effective way to launder unknown data into canonical data, and it would be called everywhere within a month precisely because it is convenient. Delete this entry when the first renderer reads its candles from here.",
  },
  "src/lib/marketData/marketObjectKinds.ts": {
    reason: "AWAITING_SURFACE",
    note:
      "The closed kind list and the shared attachment slots from SUPPORT — Truth Resolver Fidelity + MarketObject Attachments (2026-09-18) §2, the section that opens 'this is where teams usually overbuild'. It has no screen yet because the drawer it governs already exists and already renders per-kind: wiring it this week would mean rewriting MarketObjectPassportPanel in the same commit that declares the law, and the law is the part that has to be reviewable on its own. What a human loses meanwhile is nothing they had. What the house gains now is that the next kind proposed cannot arrive with its own private field — every kind carries all fifteen slots or fails the suite — and that ORDER_BLOCK, WYCKOFF, BPR and IFVG get an ANSWER ('that is a ZONE', 'that is a GAP') rather than a refusal, because a refusal is how the kind gets built anyway next quarter under a different spelling. It also deliberately exports no affirmative permission function: the canon's sharpest rejection is 'kind = permission — touched OB = GO', and the safest way to keep a shape from being read as consent is for isTradeable() to not exist. Delete this entry when the Passport reads its slots from here.",
  },
