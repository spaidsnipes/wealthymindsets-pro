...
for (const d of ["CARDS"]) {
  await p.evaluate(v => { localStorage.setItem("wm_ofAnatomyCards", "false"); localStorage.setItem("wm_ofQuestionLens", "true"); localStorage.setItem("wm_ofScaffolding", "\"OFF\""); }, d);
  await p.reload({ waitUntil: "domcontentloaded" });
  await p.waitForTimeout(14000);
  const rc = await p.evaluate(() => { const c = [...document.querySelectorAll("canvas")].find(c => c.dataset.questionLens); return c ? c.dataset.questionLens : "none"; });
  console.log(d, "receipt:", rc);
  await p.screenshot({ path: `scratchpad/shift0924/runtime_qlens3_${d}.png` });
}
