...
await p.screenshot({ path: "scratchpad/shift0924/stack_before.png" });
...
await p.getByRole("button", { name: "Move Visible Range Profile inward" }).click(); await p.waitForTimeout(300);
await p.locator('[data-testid="stack-opacity-LIVING"]').click(); await p.locator('[data-testid="stack-opacity-LIVING"]').click();
await p.locator('[data-testid="stack-arrange-bar"]').scrollIntoViewIfNeeded();
await p.screenshot({ path: "scratchpad/shift0924/stack_panel.png" });
await p.keyboard.press("Escape"); await p.waitForTimeout(1200);
console.log("lanes after:", await lanes());
await p.screenshot({ path: "scratchpad/shift0924/stack_after.png" });
