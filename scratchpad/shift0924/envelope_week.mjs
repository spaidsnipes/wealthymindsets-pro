...
await ctx.addInitScript(() => { for (const k of ["wm_ofTpoProfile","wm_ofProfileMemory","wm_ofProfileFusion","wm_ofValueMigration","wm_ofCompositeProfile","wm_absorptionAnatomy"]) localStorage.setItem(k, "false"); localStorage.setItem("wm_ofLivingProfile","true"); localStorage.setItem("wm_ofExpectedEnvelope","true"); });
const p = await ctx.newPage();
await p.goto("http://localhost:3100/charts?symbol=AAPL&tf=5m", { waitUntil: "domcontentloaded", timeout: 120000 });
await p.waitForTimeout(22000);
console.log("envelope:", await p.evaluate(() => { const c = [...document.querySelectorAll("canvas")].find(c => c.dataset.expectedEnvelope); return c ? c.dataset.expectedEnvelope : "none"; }));
await p.screenshot({ path: "scratchpad/shift0924/runtime_envelope_week.png" });
if (process.env.INSPECT) { await p.getByRole("button", { name: /inspect/i }).first().click(); await p.waitForTimeout(1500); console.log("envelope ticket:", (await p.locator('[data-inspect-envelope]').innerText().catch(() => "none")).replace(/\s+/g, " ")); await p.screenshot({ path: "scratchpad/shift0924/runtime_envelope_inspect.png" }); }
await b.close();
process.exit(0);
