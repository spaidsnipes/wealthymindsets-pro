...
import { fixtureBars, fixtureDays, fixtureDaysNow } from "./fixture.mjs";
const [,, out, ...onKeys] = process.argv;
const ALL = ["wm_fixedVP","wm_sessionVP","wm_ofLivingProfile","wm_ofTpoProfile","wm_ofStructureProfile","wm_ofProfileDna","wm_ofMarketStructure","wm_ofValueMigration","wm_ofProfileMemory","wm_ofProfileFusion","wm_ofCompositeProfile","wm_ofVisibleRangeProfile","wm_ofRegimeLighting"];
...
await ctx.route("**/api/yahoo?*type=candles*", r => r.fulfill({ status: 200, contentType: "application/json",
  body: JSON.stringify({ candles: process.env.FIXTURE === "days" ? fixtureDays() : process.env.FIXTURE === "now" ? fixtureDaysNow() : fixtureBars(), barFidelity: "INDICATIVE", sessionKnown: false }) }));
