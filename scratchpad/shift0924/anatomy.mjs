...
for (const d of ["CARDS"]) {
  await p.evaluate(v => { localStorage.setItem("wm_ofAnatomyCards", "true"); localStorage.setItem("wm_ofScaffolding", "\"OFF\""); }, d);
  await p.reload({ waitUntil: "domcontentloaded" });
  await p.waitForTimeout(14000);
  const rc = await p.evaluate(() => { const c = [...document.querySelectorAll("canvas")].find(c => c.dataset.anatomyCards); return c ? c.dataset.anatomyCards : "none"; });
  console.log(d, "receipt:", rc);
  await p.screenshot({ path: `scratchpad/shift0924/runtime_anatomy_${d}.png` });
}
