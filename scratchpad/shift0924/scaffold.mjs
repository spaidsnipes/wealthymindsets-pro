...
await ctx.addInitScript(() => {
  for (const k of ["wm_ofTpoProfile","wm_ofProfileMemory","wm_ofProfileFusion","wm_ofValueMigration","wm_ofCompositeProfile","wm_ofQuestionLens","wm_ofLivingProfile"]) localStorage.setItem(k, "false");
...
for (const d of (process.argv[2] ? process.argv[2].split(",") : ["FOUNDATION", "INTERMEDIATE", "PRO"])) {
  await p.evaluate(v => { localStorage.setItem("wm_ofScaffolding", JSON.stringify(v)); }, d);
  await p.reload({ waitUntil: "domcontentloaded" });
  await p.waitForTimeout(14000);
  const rc = await p.evaluate(() => { const c = [...document.querySelectorAll("canvas")].find(c => c.dataset.scaffolding); return c ? c.dataset.scaffolding : "none"; });
  console.log(d, "receipt:", rc);
  await p.screenshot({ path: `scratchpad/shift0924/runtime_scaffold_${d}.png` });
}
