...
import { fixtureBars, fixtureDays, fixtureDaysNow, fixtureWeekNow, fixtureExhaustNow } from "../shift0924/fixture.mjs";
...
await ctx.route("**/api/yahoo?*type=candles*", r => r.fulfill({ status: 200, contentType: "application/json",
  body: JSON.stringify({ candles: process.env.FIXTURE === "days" ? fixtureDays() : process.env.FIXTURE === "now" ? fixtureDaysNow() : process.env.FIXTURE === "week" ? fixtureWeekNow() : process.env.FIXTURE === "exhaust" ? fixtureExhaustNow() : fixtureBars(), barFidelity: "INDICATIVE", sessionKnown: false }) }));
...
const ds = await p.evaluate(() => { const c = [...document.querySelectorAll("canvas")].find(c => c.dataset.profileMemory !== undefined); return c ? { mem: c.dataset.profileMemoryGeometry, ghosts: c.dataset.sessionGhosts, living: c.dataset.livingProfileForm } : null; });
