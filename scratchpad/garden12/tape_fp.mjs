...
await ctx.addInitScript(off => { localStorage.setItem("wm_fp_enabled", "true"); localStorage.setItem("wm_footprint", "\"bid-ask\""); for (const k of off) localStorage.setItem(k, "false"); }, OFF);
...
for (const tool of (process.env.TOOLS ? process.env.TOOLS.split(",") : ["Delta Bubbles", "Big Trades"])) {
  ...
  await p.screenshot({ path: `scratchpad/garden12/fp_${tool.replace(/\s+/g, "_")}${process.env.TAG || ""}.png` });
}
await p.mouse.move(1150, 450);
...
await p.screenshot({ path: "scratchpad/garden12/fp_near.png" });
const fp = await p.evaluate(() => { const c = [...document.querySelectorAll("canvas")].find(c => c.dataset.absorption !== undefined); return c ? Object.fromEntries(Object.entries(c.dataset).filter(([k]) => /footprint|fp|semantic/i.test(k))) : {}; });
console.log("NEAR FOOTPRINT", JSON.stringify(fp));
